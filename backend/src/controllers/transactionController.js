...
const transactionController = {
    async transaction(req, res) {
        try {
            ...
            if (quantity <= 0) {
                return ResponseAPI.badRequest(res, 'Quantity must be greater than zero');
            }
            ...
            if (event.quota < quantity) {
                return ResponseAPI.conflict(res, `Insufficient tickets available. Remaining quota: ${event.quota}`);
            }
            ...
            const transactionPayload = {
                ...
                enabled_payments: ['gopay', 'shopeepay', 'ovo', 'dana', 'linkaja', 'bank_transfer', 'qris'],
            };
    ...
    async midtransStatusCallback(req, res) {
        try {
            ...
            const transaction = await DB.Transaction.findOne({ _id: transaction_id, customerId });

            if (!transaction) {
                return ResponseAPI.notFound(res, 'Transaction not found or does not belong to the current user');
            }
            ...
            switch (midtransResponse.transaction_status) {
                ...
                case 'capture':
                    ...
                    for (let i = 0; i < transaction.quantity; i++) {
                        const ticketCode = `${event.name[0]}${event.eventBy[0]}-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`;
                    ...
                    // event.reservedQuota -= transaction.quantity; // Lepaskan kuota reservasi
                    await event.save();
                    ...
                    const emailContent = `
                        ...
                        <p>Thank you for purchasing tickets for ${event.name}. Below are your ticket details:</p>
                        <ul>
                            ${tickets.map(
                                (ticket) =>
                    ...
                    `;
                    await sendMail(req.user.email, `E-Ticket for ${event.name}`, emailContent);
                    break;
                ...
                return ResponseAPI.success(res, {
                    transaction,
                    midtransResponse,
                }, 'Transaction status is awaiting payment');
            }
            ...
            return ResponseAPI.serverError(res, error.message || 'Error checking payment status');
        }
    },
    ...
    async getTransactionHistory(req, res) {
        try {
            ...
            return ResponseAPI.success(res, { history }, 'Transaction history retrieved successfully');
        } catch (error) {
            console.error('Error retrieving transaction history:', error);
            return ResponseAPI.serverError(res, error.message || 'Error retrieving transaction history');
        }
    },
